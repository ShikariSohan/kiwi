...
const FunnyImage = (props: any) => {
  const preload = (p5:p5Types) => {
    orgImg = p5.loadImage(`/preAsset/pinwheel.jpg`);
  }
...
const draw = (p5:p5Types) => {
  ...
  let img = p5.createImage(source.width, source.height);
    img.copy(source, 0, 0, source.width, source.height, 0, 0, source.width, source.height);
    img.resize(ww, hh);
    ...
    case 'wavey':
        ...
        for (let x = 0; x < img.width; x += 10) {
            for (let y = 0; y < img.height; y += 10) {
                ...
                p5.curve(
                xPos,yPos,p5.sin(xPos) * 10,p5.cos(yPos) * p5.sin(xPos) * 6,0,0,p5.cos(yPos) * p5.sin(xPos) * 30,
    ...
    case 'ascii':
        ...
        for (let x = 0; x < img.width; x += 10) {
            for (let y = 0; y < img.height; y += 10) {
                ...
                const asciiIndex = p5.floor(p5.map(brightness, 0, 255, ascii.length,0));
                // draw the character
                p5.textSize(10);
                p5.fill(255);
